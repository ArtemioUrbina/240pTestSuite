...
void DrawCheckBoard()
{
	...
	while(!exit)
	{
		if(loadvram)
		{
			...
			VDP_fillTileMapRect(APLAN, TILE_ATTR(PAL1, 0, 0, 0) + TILE_USERINDEX, 0, 0, 320 / 8, getVerticalRes() / 8);
			VDP_End();
			loadvram = 0;
		}
		...
		if(alternate)
		{
			...
			{
				VDP_Start();
				VDP_fillTileMapRect(APLAN, TILE_ATTR(PAL1, 0, 0, 0) + ind, 0, 0, 320 / 8, getVerticalRes() / 8);
				VDP_End();
				field = 0;
			}
		}
...
void DrawStripes()
{
	...
	while(!exit)
	{
		...
		if(alternate || redraw)
		{
			...
			{
				if(vertical)
					VDP_fillTileMapRect(APLAN, TILE_ATTR(PAL1, 0, 0, 0) + ver1, 0, 0, 320 / 8, getVerticalRes() / 8);
				else
					VDP_fillTileMapRect(APLAN, TILE_ATTR(PAL1, 0, 0, 0) + hor1, 0, 0, 320 / 8, getVerticalRes() / 8);
				field = 1;
			}
			else
			{
				if(vertical)
					VDP_fillTileMapRect(APLAN, TILE_ATTR(PAL1, 0, 0, 0) + ver2, 0, 0, 320 / 8, getVerticalRes() / 8);
				else
					VDP_fillTileMapRect(APLAN, TILE_ATTR(PAL1, 0, 0, 0) + hor2, 0, 0, 320 / 8, getVerticalRes() / 8);
				field = 0;
			}
			VDP_End();
			redraw = 0;
...
void PassiveLagTest()
{
	...
	while(!exit)
	{
		if(loadvram)
		{
			...
			VDP_fillTileMapRect(BPLAN, TILE_ATTR(PAL2, 0, 0, 0) + solid, 0, 0, 320 / 8, getVerticalRes() / 8);
